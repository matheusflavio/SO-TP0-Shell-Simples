...
/* MARK NAME Matheus Flávio Gonçalves Silva */
...
runcmd(struct cmd *cmd)
{
  ...
  case '<':
    ...
    close(rcmd->fd); // Fechando descritor
    open(rcmd->file,rcmd->mode, 0420); // Abrindo arquivo E/S e definindo modo
    /* MARK END task3 */
    runcmd(rcmd->cmd);
    break;
  ...
  case '|':
     ...
     * comando com pipes. */
    if(pipe(p) == -1) { // Verificando código de erro no pipe
      perror("Erro no pipe");
      exit(1);
    }
    switch(r = fork1()) {
      case -1: // Retorno de erro
  ...
  while(getcmd(buf, sizeof(buf)) >= 0){
    ...
    */
    if(buf[0] == 'c' && buf[1] == 'd' && buf[2] == ' '){
      buf[strlen(buf)-1] = 0;
      if(chdir(buf+3) < 0)
        fprintf(stderr, "Erro: Diretório não encontrado ou inexistente\n");
      continue;
    }
    /* MARK END task1 */
